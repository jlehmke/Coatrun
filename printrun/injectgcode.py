...
def injector(gcode, viz_layer, layer_idx):
    ...
    MacroEditor("Inject G-Code at layer %d (Z = %.03f)" % (viz_layer, z), "", cb, True)
...
def injector_edit(gcode, viz_layer, layer_idx):
    ...
    MacroEditor("Edit G-Code of layer %d (Z = %.03f)" % (viz_layer, z), lines, cb, True)
...
def inject(gcode, viz_layer, layer_idx, toadd):
    ...
    logging.info("Successfully injected %d lines at beginning of layer %d" % (nlines, viz_layer))
...
def rewritelayer(gcode, viz_layer, layer_idx, toadd):
    ...
    logging.info("Successfully edited layer %d (which now contains %d lines)" % (viz_layer, nlines))
